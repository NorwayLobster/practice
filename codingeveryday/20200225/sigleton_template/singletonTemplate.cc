...
//lazy mode + pthread_once()
			pthread_once(&once_control,createInstance);
			return _pInstance;
	}
	static void createInstance(void){
		if(NULL==_pInstance){
//自动析构 atexit(destory);
//1. 需要使用_auto:静态对象成员，不需要创建Singleton实例对象，就可以使用
...
//eager mode  也可以main()刚开始就getInstance(), 创建线程之前即可//error: 初始化静态成员时
	Student * pSudent =	Singleton<Student>::getInstance();
	Student * pSudent1 =	Singleton<Student>::getInstance();
