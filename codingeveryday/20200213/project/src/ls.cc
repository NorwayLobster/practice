...
//note: opendir打开的绝对路径，相对路径必须拼接成用户的绝对路径；stat()也是绝对路径,
//待改造
int ls(char * p_path,char *response){
	...
	char * ret_pwd=getcwd(pwd,sizeof(pwd));
	printf("p_pwd:%s\n",pwd);
	assert(NULL!=ret_pwd);
	int ret_chdir=chdir(p_path);
	assert(-1!=ret_chdir);
	ret_pwd=getcwd(pwd,sizeof(pwd));
	printf("p_pwd:%s\n",pwd);

//	printf("response sizeof:%ld\n\n",sizeof(response));
	int  ret;
	DIR * p_dir;
...
shared_ptr->st_mtime,shared_ptr->st_ctime);
	...
	//	printf("response sizeof:%ld\n",sizeof(response));
	//	printf("%s\n",response);
	//	char *p11=response+strlen(response);	
	//error:	char *p11=response+sizeof(response);	
//		ret=sprintf(p11,"%s %4ld %8s %8s %10ld %10ld %10ld %10ld %s %-s\n" ,s_rwx.c_str(),shared_ptr->st_nlink,p_passwd->pw_name,p_group->gr_name,shared_ptr->st_rdev,shared_ptr->st_size,shared_ptr->st_blksize,shared_ptr->st_blocks,buf+4,p_dirent->d_name);
		ret=sprintf(response+strlen(response),"%s %4ld %8s %8s %10ld %10ld %10ld %10ld %s %-s\n" ,s_rwx.c_str(),shared_ptr->st_nlink,p_passwd->pw_name,p_group->gr_name,shared_ptr->st_rdev,shared_ptr->st_size,shared_ptr->st_blksize,shared_ptr->st_blocks,buf+4,p_dirent->d_name);
		assert(ret>=0);
...
int main(int argc, char**argv){
	...
	char response[4095]={0};
	printf("response sizeof:%d\n",sizeof(response));
	printf("response sizeof:%ld\n",sizeof(response));
//	ls(".",response);
	ls("..",response);
	return 0;
