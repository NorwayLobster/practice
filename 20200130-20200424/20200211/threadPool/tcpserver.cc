...
int pfd[2];
void sig(int signum){
	printf("%d is coming,threadid:%ld\n",signum,pthread_self());
	int ret;
...
int main(int argc,char**argv){
	...
	ThreadPool Tp;
	threadPool_init(&Tp,task_handler,threadNum,capacity);
	cout<<"in main size="<<Tp.taskqueue.sockfdqueue.size()<<endl;
	printf("in main size:%lu\n", Tp.taskqueue.sockfdqueue.size());
	printf("in main size:%ld\n", Tp.taskqueue.sockfdqueue.size());
	printf("in main size:%d\n", (int)Tp.taskqueue.sockfdqueue.size());
	createChildThreads(&Tp,threadNum);
	...
	int maxNoListen=threadNum+1;//监听sockpair，sfd,和新的网络连接//????????????????????????????????????
		...
		for(int i=0;i<retval;++i){
			...
			if(sockfd==pfd[0]){
				...
				printf("got it,exit,signum:%d\n",signum);
				 ...
				 Tp.exit_flag=1;
				 Tp.taskqueue.sockfdqueue.push(newfd);//把标准输入放入队列
				 pthread_mutex_unlock(&Tp.mutex);
				 pthread_cond_signal(&Tp.cond);

				//等待子进程存盘，完成现有业务等
				for(auto i=0;i<threadNum;++i){
					printf("before pthread_join,%ld\n",Tp.pNodeArr[i].pid);
					int retval;
					pthread_join(Tp.pNodeArr[i].pid,(void**)&retval);//等待子进程干完活，优雅退出
				}
				_exit(0);
			}
			else if(sfd==sockfd){
				...
				int stepin_flag=0;
				 ...
				 pthread_mutex_lock(&Tp.mutex);
				 if((int)Tp.taskqueue.sockfdqueue.size()<Tp.taskqueue.capacity){
					 Tp.taskqueue.sockfdqueue.push(newfd);
					 stepin_flag=1;
				 }
				 pthread_mutex_unlock(&Tp.mutex);
