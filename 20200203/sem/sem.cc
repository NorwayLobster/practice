...
//2.semctl()
	...
	cmd=SETVAL;
	int sem_no_th=0;
//union semun形参是，怎么传实参
	int val=1; //直接传union该元素 val
...
//5.semctl() IPC_STAT
	struct semid_ds buf;	//直接传union该元素 &buf
	memset(&buf,0,sizeof(buf));
	ret=semctl(semid,0,IPC_STAT,&buf);
	PERROR(-1,ret,"semctl() IPC_STAT");
	printf("sem_perm.gid:%d,sem_perm.uid:%d,sem_perm.mode:%o\n",buf.sem_perm.gid,buf.sem_perm.uid,buf.sem_perm.mode);

//5.semctl() IPC_SET //设置属性，整个sem集合不管多少个sem，都设置成相同的属性
//union semun形参是，怎么传实参
	buf.sem_perm.mode=0666;
	ret=semctl(semid,0,IPC_SET,&buf);
	PERROR(-1,ret,"semctl() IPC_SET");
