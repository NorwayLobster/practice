...
//	manacherString.reserve(len*2+1);
	...
	for(int i=0;i<len;++i){
		...
		firstReachEndRadius=Radius[i];
		if(curMostRightBoundary==len){//first go to end
			cout<<"radius[i]"<<Radius[i]<<endl;
			break;
		}
	}
	cout<<"s:"<<s<<endl;
	cout<<"s:"<<*(s.end()-firstReachEndRadius)<<endl;
	return string(s.begin(),s.end()-firstReachEndRadius+1);
//	return maxRadius-1;//
}
int main(){
	string s("abcbab");
